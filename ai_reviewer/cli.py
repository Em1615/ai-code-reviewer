...
def _required_env(name: str) -> str:
    ...
    if not value:
        print(f"Missing required enviromaent variable: {name}", file = sys.stderr)
        sys.exit(1)
    return value
...
def run() -> None:
    ...
    for f in changed_files:
        ...
        try:
            raw_ourput = openrouter.review_with_model(
                model = model,
            ...
            print(f"Review failed for {f.filename}: {exc}", file = sys.srderr)
            continue

        issues = parser.parse_issues(raw_ourput)
